...
public class WebServer {
  ...
  public void initialize() {
    ...
    //Shows the Checkers game Sign out page.
    get(SIGNOUT_URL, new GetSignOutRoute(templateEngine));

    //validates user submitted moves
    post(VALIDATE_MOVE, new PostValidateMoveRoute());

    //submits valid user turns
    post(SUBMIT_TURN, new PostSubmitTurnRoute(playerLobby));

    //undoes validated user moves
    ...
    //user resigns game
    post(RESIGN_GAME, new PostResignGameRoute());
    
    //Posts the user sign in data to the Sign in page.
    post(SIGNIN_URL, new PostSigninRoute(templateEngine, playerLobby));
    
    //Shows the Sign out page.
